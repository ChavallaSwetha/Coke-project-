...
public class ComparingStoreLevelPremiseDataAndWritingXL {
	...
	private void compareAndWriteForKPI(String storeName, PremiseKPIs kpi, int z1, UIData dataUI, XLData xldata,
		...
		if (valueFromXl == null) {
			result1 = new Label(traditionalProperties.get(kpi)[2], z1 + 1, "Missing Data in XL");
		} else {
			...
			if (diffBetweentotalUIAndIceValueXL >= 0.5) {
				result1 = new Label(traditionalProperties.get(kpi)[2], z1 + 1, "Mismatch");

			} else {
				result1 = new Label(traditionalProperties.get(kpi)[2], z1 + 1, "Match");
			}
		}
