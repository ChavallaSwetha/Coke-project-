...
public class PopprobeComparingCountryLevelDataWithCooler {
	...
	public static void main(String[] args) throws InterruptedException, BiffException, IOException, RowsExceededException, WriteException {
		...
		String freshSt = columns.get(6).getText();
		Float freshFt = Float.parseFloat(freshSt);
		...
		for (int r = 0; r < rowsXL; r++) {
			if ((kpiXL[r]).equals(mpa)) {
				...
				if (pID.equals(withCooler)) {
					...
					Label mpaAdd = new Label(5, 7, mpaSt);
				...
				Label resultAdd = new Label(6, 7, result);
				...
				if (pID.equals(withCooler)) {
					...
					writeSh.addCell(channelAdd);
					Label soviAdd = new Label(5, 8, soviSt);
				...
				Label resultAdd = new Label(6, 8, result);
				writeSh.addCell(resultAdd);
				...
				if (pID1.equals(withCooler)) {
					...
					writeSh.addCell(channelAdd);
					Label refAdd = new Label(5, 9, refSt);
				...
				Label resultAdd = new Label(6, 9, result);
				writeSh.addCell(resultAdd);
				...
				if (pID1.equals(withCooler)) {
					...
					writeSh.addCell(channelAdd);
					Label communionAdd = new Label(5, 10, commexSt);
				...
				Label resultAdd = new Label(6, 10, result);
				writeSh.addCell(resultAdd);
				...
				if (pID1.equals(withCooler)) {
					...
					writeSh.addCell(channelAdd);
					Label priceAdd = new Label(5, 11, priceSt);
				...
				Label resultAdd = new Label(6, 11, result);
				writeSh.addCell(resultAdd);
				...
				writeSh.addCell(channelAdd);
				Label totalAdd = new Label(5,12, total);
			...
			Label resultAdd = new Label(6, 12, result);
			writeSh.addCell(resultAdd);
		}
	}
